...
def fetch_latest_videos(channel_id: str, max_age_hours: float = 24.0) -> list[dict]:
    """
    ...
    """
    # Channel ID validation (should start with UC and be 24 chars long)
    ...
    try:
        ...
        for entry in feed.entries:
            ...
            # Check if video was published within the time window
            age = now_utc - published_dt
            if age.total_seconds() <= max_age_hours * 3600:
                # Extract video ID from feed entry ID (typically: yt:video:VIDEO_ID)
                video_id = None
